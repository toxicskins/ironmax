...
export async function POST(req: Request) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const currentCard = shuffled(next, freshDeck())[0];

    await tx.wallet.update({ where: { userId }, data: { coins: { decrement: stake } } });
    await tx.transaction.create({
      data: { userId, type: "BET", coinsDelta: -stake, note: "Bet on hilo" },
    });

    const round = await tx.hiLoRound.create({
      data: { userId, stake, currentCard, serverSeed, clientSeed, nonce: BigInt(nonce) },
    });
  ...
  return NextResponse.json({ roundId: result.id, stake: result.stake, current: result.currentCard });
}
