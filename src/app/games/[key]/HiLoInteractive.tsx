...
export function HiLoInteractive({ minStake, maxStake, initialCoins }: {
  ...
  useEffect(() => {
    ...
    fetch("/api/games/hilo/active")
      .then((r) => r.json())
      .then((body) => {
        if (body.round) {
          setRoundId(body.round.roundId);
          setStake(body.round.stake);
          setCurrent(body.round.current);
          setPhase("active");
        }
      })
  ...
  async function pick(g: "higher" | "lower") {
    if (loading) return;
    setLoading(true);
    setError(null);
    setGuess(g);
    const res = await fetch("/api/games/hilo/pick", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ roundId, guess: g }),
    });
    const body = await res.json();
    setLoading(false);
    if (!res.ok) { setError(body.error ?? "Pick failed"); return; }
    setNextCard(body.nextCard);
    setWin(body.win);
      ...
      <div className="relative p-2 rounded-2xl bg-zinc-900">
        ...
        >
          {!loggedIn ? (
            ...
            <>
              <div className="flex gap-3 sm:gap-4 justify-center relative z-10">
                {current && <CardRow cards={[current]} resultKey={`${roundId}-current`} />}
                {nextCard ? (
                  <CardRow cards={[nextCard]} resultKey={`${roundId}-next`} />
                ) : (
                  <CardBack index={1} />
                )}
              </div>
              ...
              {phase === "finished" && (
                <div className="flex flex-col items-center gap-1 relative z-10">
                  <p className={`text-2xl font-extrabold ${win ? "text-emerald-400" : "text-red-400"}`}>
                    {win ? `+${payout} points` : "No win this time"}
                  </p>
                  <p className="text-sm text-zinc-500">You called {guess}</p>
                </div>
              )}
            </>
          )}
